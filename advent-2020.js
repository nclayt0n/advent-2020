...
//Puzzle 1
//find the 2 numbers that equal 2020, then multiply them together.
const findSum = (arr) => {
  let list = [];
  arr.map((r) => {
...
document.getElementById(
  "DayOnePuzzleOne"
).innerText = `Puzzle 1: ${DayOnePuzzleOne}`;
...
document.getElementById(
  "DayOnePuzzleTwo"
).innerText = `Puzzle 2: ${DayOnePuzzleTwo}`;
...
const howManyPasswordsAreValid = (arr) => {
  ...
  inputs.map((input) => {
    ...
    let occurences = input.password.split("").filter((l) => l === input.letter)
      .length;
...
document.getElementById(
  "DayTwoPuzzleOne"
).innerText = `Puzzle 1: ${DayTwoPuzzleOne}`;
...
const validPasswordsCount = (arr) => {
  ...
  inputs.map((input) => {
    ...
    if (
      splitPassword[range[0] - 1] === input.letter ||
      splitPassword[range[1] - 1] === input.letter
    ) {
      if (
        splitPassword[range[0] - 1] === input.letter &&
        splitPassword[range[1] - 1] === input.letter
...
document.getElementById(
  "DayTwoPuzzleTwo"
).innerText = `Puzzle 2: ${DayTwoPuzzleTwo}`;
